...
df_all_vars_30m_FV = pd.read_csv(
    f"{paths['Save_Processed']}Processed_Duke_Met_Data_All_Vars_30m_FV.csv", index_col=0
)
df_all_vars_30m = pd.read_csv(
    f"{paths['Save_Processed']}Processed_Duke_Met_Data_All_Vars_30m.csv", index_col=0
)
...
df_h["DTIME"] = (df_all_vars_30m["Time"] / (24 * 60) + df_all_vars_30m["DOY"]).astype(
    float
)
...
for k_var in ["Rainf"]:
    ...
    factor_multiple = 1 / (
        60 * 30
    )  # From per day to per second for 48 timesteps in a day i.e. 24*60*60/48
...
unit_row = pd.Series(
    [dict_units.get(col, "") for col in df_h.columns], index=df_h.columns
)
...
for col_name in ["PAR", "RH", "Tair", "Rainf", "VPD"]:
    ...
    filter_tmp = df_h_save[
        (df_h_save["YEAR"] == year_tmp)
        & (df_h_save["DOY"] == doy_tmp)
        & (df_h_save["HRMIN"] == time_tmp)
    ]
...
df_d_wTime["Rainf"] = (
    df_h_wTime[["YEAR", "DOY", "Rainf"]].resample("D").sum()["Rainf"]
    * factor_multiple_s2d
)
# For PAR we need to take sum
df_d_wTime["PAR"] = (
    df_h_wTime[["YEAR", "DOY", "PAR"]].resample("D").sum()["PAR"]
    * factor_multiple_s2d
    * 10 ** (-6)
)
# For Rn we need to take sum
df_d_wTime["Rn"] = (
    df_h_wTime[["YEAR", "DOY", "Rn"]].resample("D").sum()["Rn"]
    * factor_multiple_s2d
    * 10 ** (-6)
)
# For Ndep we need to take sum
df_d_wTime["Ndep"] = (
    df_h_wTime[["YEAR", "DOY", "Ndep"]].resample("D").sum()["Ndep"]
    * factor_multiple_s2d
)
...
# Adding the unit row below first row
unit_row = pd.Series(
    [dict_units_d.get(col, "") for col in df_d.columns], index=df_d.columns
)
...
# Adding the unit row below first row
unit_row = pd.Series(
    [dict_units_y.get(col, "") for col in df_y.columns], index=df_y.columns
)
...
ds["HRMIN"].attrs[
    "long_name"
] = "Hour:minute - marked at the middle of measurement interval with last two digits as minute"
...
ds.attrs[
    "title"
] = "Half-hourly forcing data from DUKE Forest FACE, North Carolina, USA"
ds.attrs[
    "history"
] = "File Origin - This file was created at Oak Ridge National Laboratory for FACE model data synthesis"
ds.attrs["creation_date"] = "Sep 25, 2023"
ds.attrs[
    "contact"
] = "Bharat Sharma (sharmabd@ornl.gov), Anthony Walker (walkerp@ornl.gov)"
