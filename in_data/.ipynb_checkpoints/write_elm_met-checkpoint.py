...
def bypass_format(filename, met_data, lat, lon, startyear, endyear, edge=0.1, time_offset=0, calc_qbot = False, calc_lw = False, zbot=10):
  ...
  all_hourly.createDimension('time', nt)
  all_hourly.createDimension('gridcell',1)
  all_hourly.createDimension('scalar',1)
  for v in metvars:
    all_hourly.createVariable(v, 'f', ('gridcell','time',))
  ...
  if (calc_qbot):
    all_hourly.createVariable('QBOT','f',('gridcell','time',))
    all_hourly.createVariable('VPD','f',('gridcell','time',))
  ...
  if (calc_lw):
    all_hourly.createVariable('FLDS','f',('gridcell','time',))
    stebol = 5.67e-8
    mye = esat(all_hourly['TBOT'][0,:]-273.15) * all_hourly['RH'][0,:]/100.
  ...
  if (not 'ZBOT' in metvars):
    all_hourly.createVariable('ZBOT','f',('gridcell','time',))
  ...
  output_data.createVariable('time', 'f8', 'time')
  output_data.variables['time'].long_name='observation time'
  output_data.variables['time'].units='days since '+str(startyear)+'-01-01 00:00:00'
  output_data.variables['time'].calendar='noleap'
  n_years = endyear-startyear+1
  output_data.variables['time'][:] = np.cumsum(np.ones([int(n_years*365*npd)], np.float)/npd)-0.5/npd
